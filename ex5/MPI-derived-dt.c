...
int main(int argc, char** argv) {
    ...
    if (my_rank == 0) {
        ...
        if (fp == NULL) {
            ...
            fclose(fp);
            if(comm_sz!=total_students){
                    printf("Error:The procces number is not equal to the total students\n");
    ...
    if (my_rank == 0) {
        ...
        for (int i = 0; i < total_students; i++) {
            fprintf(out,"Name:%s | RollNumber: %d |  Grade: %c\n", all_students[i].name, all_students[i].roll_no, all_students[i].grade);
        }
        fclose(out);
        printf("Processed %d students. Results in output.txt\n", total_students);
        free(all_students);
    }
